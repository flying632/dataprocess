...
object MetricToEs {
  ...
  val elasticsearchService:ElasticsearchService = new ElasticsearchServiceImpl()
  def main(args: Array[String]): Unit = {
    ...
    kafkaStream.foreachRDD(r => {
      val ESRdd = r.map(x=>{
        val result = resolveValue(x.value())
        result
      })
      import org.elasticsearch.spark._
      ESRdd.saveToEs("/fylmetric/message")
      //检查是否存在异常指标
      elasticsearchService.checkAll()
    })
  ...
  def resolveValue(strValue:String) : Object = {
    val value = JSON.parseObject(strValue)
